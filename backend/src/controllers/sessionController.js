    ...
    try{
        const userId = req.user._id
           ...
           const sessions = await Session.find({
            status:"completed",
            $or: [{host:userId},{partcicipant:userId}],
           })
           .sort({ createdAt: -1})
           .limit(20);
           res.status(200).json({sessions})
...
try{
       ...
       if(session.host.toString() != userId.toString()){
        return res.status(403).json({message:"Only the hist can end the session"})
       }

       //check if session is already completed
       if(session.status === "completed"){
        return res.status(400).json({message:"Session is already comleted "});
       }
